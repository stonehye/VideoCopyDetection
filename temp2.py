...
if __name__ == '__main__':
    ...
    feature_path= '/nfs_shared_/hkseok/minmax/vcdb_core_minmax_545-0-mobilenet_avg-10-segment_maxpooling'
    # feature_path = '/nfs_shared_/hkseok/local/vcdb_core-0-mobilenet_avg-32-segment_maxpooling'
    ...
    for n, (query, gt) in enumerate(annotation.items(), start=1):
        ...
        print(n, '==')
        print(f'query : {query} qid : {q_id} loc :{loc[q_id]} gt_count: {len(gt)}')
        video_avg_rank = []
        for idx, i in enumerate(I):
            count = 0
            query_range = db_intervals[query][idx]
            tempgt = [g for g in gt if is_overlap(query_range, [g[3], g[4]])]  # query 세그먼트에 해당하는 gt들
            print(f'{idx} 번쨰 쿼리 feature({query_range})가 포함된 정답 세그먼트는 {len(tempgt)} 개 , {tempgt}')
            if tempgt == []:
                continue
            true_tempgt = []
            ranks = []
            for rank, fid in enumerate(i):
                ...
                feature_range = db_intervals[ref_video_name][feature_order]

                # 검사해야하는 gt들, checkgt reference video의 세그먼트가 rank 안에 존재해야함
                checkgt = [g for g in tempgt if g[2] == ref_video_name]
                ...
                if isHit:
                    ...
                    ranks.append(rank)
                    print(
                        f'{count} : {rank} 등의 feature는 {ref_video_id}번째 비디오({ref_video_name})의 {feature_order}번 째 샷({feature_range})은 hit 다')

            true = [t for t in tempgt if t in true_tempgt]
            print(len(true))
            exit()
            precision = count / topk * 100
            recall = len(true) / len(tempgt) * 100
    ...
    print(topk, np.average(np.array(total_avg)), np.average(np.array(total_recall)),
          np.average(np.array(total_precision)))
    ...
    #     candidate = tn.fit()
    #     # print(candidate)
    #     candidate = idx2time(query, vcdb_core_video, candidate, db_intervals)
    ...
    #     print(f'{query}: {f:.4f} {p:.4f} {r:.4f} ({ff:.4f} {pp:.4f} {rr:.4f}) {a:>5d}({aa:>3d}) {b:>5d}({bb:>3d}) {c:>5d}({cc:>3d}) {d:>5d}({dd:>3d})')
    #
    #     # exit()
    # p = a / (b + 1e-12)
    # r = c / (d + 1e-12)
