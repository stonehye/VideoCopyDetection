...
if __name__ == '__main__':
    ...
    dataset_dir = '/nfs_shared_/hkseok/BOW/multiple/simulated_videos3/'
    ...
    def test(topk, tn_param, query_path):
        ...
        candidate = [[x[0], Period(x[1].start * 5, x[1].end * 5), Period(x[2].start * 5, x[2].end * 5), x[3], x[4]] for x in candidate if x[0] <= 527 or os.path.splitext(str(vcdb_core_video[x[0]]))[0]==query_video_name]
        ...
        for v in candidate:
            ...
            for n in new_list2:
                if n[0] == v[0]:
                    is_in = True
            if not is_in:
                new_list2.append([v[0], vcdb_core_video[v[0]], v[1], v[2]])
    ...
    for t in transform:
        for l in level:
            dir = os.path.join(dataset_dir, t, l)
            ...
            if os.path.isdir(dir):
                ...
                rank_list = []
                for v in video_list:
                    rank = test(topk=2000, tn_param=[20, 3, 0.3], query_path=v)
                    if rank != -1:
                        sum += 1
                        rank_sum += rank
