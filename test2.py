...
basename = '/nfs_shared_/hkseok/' + 'vcdb_core-1-res50_avg-5-segment_maxpooling'
...
for video in glob.glob('/nfs_shared_/hkseok/VCDB/videos/core/*'):
    ...
    # query_video_features = extract_segment_fingerprint(video,decode_rate,decode_size,transform,cnn_model,aggr_model,group_count)
    ...
    D, I = l2index.search(query_video_features, k=10)
    ...
    for can in candidate:
        ...
        query_start = str(datetime.timedelta(seconds=can[1][0] * decode_rate * group_count))
        query_end = str(datetime.timedelta(seconds=can[1][1] * decode_rate* group_count))
        ref_start = str(datetime.timedelta(seconds=int(can[2][0]) * decode_rate* group_count))
        ref_end = str(datetime.timedelta(seconds=int(can[2][1]) * decode_rate* group_count))
